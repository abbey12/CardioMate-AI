...
function std(xs: number[], mu: number): number {
  const v =
    xs.reduce((acc, x) => acc + (x - mu) * (x - mu), 0) /
...
function zNormalize(xs: number[]): { normalized: number[]; mu: number; sigma: number } {
  const mu = mean(xs);
  const sigma = std(xs, mu) || 1;
  return { normalized: xs.map((x) => (x - mu) / sigma), mu, sigma };
}

function detectRPeaksSimple(
  normalized: number[],
  sampleRateHz: number
): number[] {
  ...
  const diffs = normalized.map((x, i) =>
    i === 0 ? 0 : Math.abs(x - normalized[i - 1])
  );
...
function estimateHeartRateBpm(
  ...
  if (rPeakIndices.length < 2) return undefined;
  const rr: number[] = [];
  for (let i = 1; i < rPeakIndices.length; i++) {
    rr.push((rPeakIndices[i] - rPeakIndices[i - 1]) / sampleRateHz);
  }
  const rrMean = mean(rr);
  if (!Number.isFinite(rrMean) || rrMean <= 0) return undefined;
  return Math.round((60 / rrMean) * 10) / 10;
}

export function preprocessEcg(signal: EcgSignal): {
  cleaned: number[];
  normalized: number[];
  summary: EcgPreprocessSummary;
} {
  ...
  const estimatedHeartRateBpm = estimateHeartRateBpm(
    rPeakIndices,
    signal.sampleRateHz
  );

  const durationSec = xs.length / signal.sampleRateHz;
  const summary: EcgPreprocessSummary = {
