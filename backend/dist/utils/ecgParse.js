...
export function parseEcgFromCsv(csvText, sampleRateHz) {
...
export function parseEcgFromJson(jsonText) {
    const parsed = JSON.parse(jsonText);
    const data = JsonEcgSchema.parse(parsed);
    return {
        lead: data.lead,
        sampleRateHz: data.sampleRateHz,
        samples: data.samples.map((s) => typeof s === "number" ? { v: s } : { t: s.t, v: s.v })
    };
}
